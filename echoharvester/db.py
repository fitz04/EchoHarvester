...
class Database:
    ...
    async def _migrate_schema(self) -> None:
        ...
        for col_sql in [
            "ALTER TABLE segments ADD COLUMN approved_at TIMESTAMP",
            "ALTER TABLE segments ADD COLUMN approved_by TEXT DEFAULT 'user'",
        ]:
            try:
            ...
            await conn.execute(
                ...
                ON CONFLICT(id) DO UPDATE SET
                    ...
                    normalized_text = CASE
                        WHEN segments.original_text != excluded.original_text
                        THEN NULL ELSE segments.normalized_text END,
                    cer = CASE
                        WHEN segments.original_text != excluded.original_text
                        THEN NULL ELSE segments.cer END
                """,
                (
                    segment_id,
                    media_id,
    ...
    async def bulk_update_segments(
        ...
        For large batches (>500), splits into chunks to avoid
        SQLite variable limit issues.
        """
        if not segment_ids:
            return
    ...
    async def get_stats(self) -> dict:
        ...
        cursor = await self.conn.execute(
            ...
            SELECT
                ...
                AVG(cer) as avg_cer
            FROM segments
            WHERE status IN ('gpu_pass', 'exported', 'approved')
            """
        )
        row = await cursor.fetchone()
        ...
        # CER distribution (passed only — legacy)
        cursor = await self.conn.execute(
            """
            SELECT
                CASE
                ...
                COUNT(*) as count
            FROM segments
            WHERE cer IS NOT NULL AND status IN ('gpu_pass', 'exported', 'approved')
            GROUP BY cer_range
            ORDER BY cer_range
            """
        )
        ...
        # CER distribution (all validated segments — finer ranges)
        cursor = await self.conn.execute(
            """
            SELECT
                CASE
                    ...
                    WHEN cer < 0.10 THEN '5-10%'
                    WHEN cer < 0.15 THEN '10-15%'
                    WHEN cer < 0.20 THEN '15-20%'
                    WHEN cer < 0.30 THEN '20-30%'
                    WHEN cer < 0.50 THEN '30-50%'
                    ELSE '50%+'
                END as cer_range,
                COUNT(*) as count
            FROM segments
            WHERE cer IS NOT NULL
              AND status IN ('gpu_pass', 'gpu_reject', 'exported')
            GROUP BY cer_range
            """
        )
        stats["cer_distribution_all"] = {
            row["cer_range"]: row["count"] for row in await cursor.fetchall()
        }

        # Reject reasons (normalized to prefix before ':')
        cursor = await self.conn.execute(
            """
        ...
        for row in await cursor.fetchall():
            prefix = row["reject_reason"].split(":")[0]
            raw_reasons[prefix] = raw_reasons.get(prefix, 0) + row["count"]
        stats["reject_reasons"] = raw_reasons
        ...
        cursor = await self.conn.execute(
            ...
            SELECT
                ...
                COUNT(*) as total,
                SUM(CASE WHEN s.status IN ('gpu_pass', 'exported', 'approved') THEN 1 ELSE 0 END) as passed,
                SUM(CASE WHEN s.status = 'approved' THEN 1 ELSE 0 END) as approved,
                SUM(CASE WHEN s.status LIKE '%reject%' THEN 1 ELSE 0 END) as rejected,
                AVG(CASE WHEN s.cer IS NOT NULL THEN s.cer END) as avg_cer,
                SUM(CASE WHEN s.status IN ('gpu_pass', 'exported', 'approved') THEN s.duration_sec ELSE 0 END) as pass_duration
            FROM segments s
        ...
        stats["per_media"] = [dict(row) for row in await cursor.fetchall()]
    ...
    async def get_recent_segments(self, limit: int = 20) -> list[dict]:
        ...
        cursor = await self.conn.execute(
            ...
            FROM segments s
            JOIN media_items m ON s.media_id = m.id
            WHERE s.status IN ('gpu_pass', 'gpu_reject', 'exported', 'approved')
