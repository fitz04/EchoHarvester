...
try:
    ...
    custom_fwd = _ft.partial(_custom_fwd, device_type="cuda")
    custom_bwd = _ft.partial(_custom_bwd, device_type="cuda")
except ImportError:
    from torch.cuda.amp import custom_fwd, custom_bwd

# Local shim for torch_autocast (replaces icefall.utils dependency)
def torch_autocast(enabled: bool = True, **kwargs):
    """Shim for icefall's torch_autocast."""
    return torch.amp.autocast(device_type="cuda", enabled=enabled, **kwargs)
...
class ActivationDropoutAndLinearFunction(torch.autograd.Function):
    ...
    def backward(ctx, ans_grad: Tensor):
        ...
        if _k2_available:
            ...
            # Fallback: compute forward and derive manually
            def _swoosh_l_forward_and_deriv(x):
                x_f = x.float() if x.dtype != torch.float32 else x
                zero = torch.tensor(0.0, dtype=x_f.dtype, device=x_f.device)
                y = torch.logaddexp(zero, x_f - 4.0) - 0.08 * x_f - 0.035
                deriv = torch.sigmoid(x_f - 4.0) - 0.08
                return y.to(x.dtype), deriv.to(x.dtype)

            def _swoosh_r_forward_and_deriv(x):
                x_f = x.float() if x.dtype != torch.float32 else x
                zero = torch.tensor(0.0, dtype=x_f.dtype, device=x_f.device)
                y = torch.logaddexp(zero, x_f - 1.0) - 0.08 * x_f - 0.313261687
                deriv = torch.sigmoid(x_f - 1.0) - 0.08
                return y.to(x.dtype), deriv.to(x.dtype)

            forward_and_deriv_activation_dict = {
                "SwooshL": _swoosh_l_forward_and_deriv,
                "SwooshR": _swoosh_r_forward_and_deriv,
            }
