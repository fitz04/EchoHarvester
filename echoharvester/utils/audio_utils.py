...
async def convert_to_wav(
    ...
    for attempt in range(1, max_retries + 1):
        ...
        if attempt < max_retries:
            logger.warning(
                f"ffmpeg conversion attempt {attempt}/{max_retries} failed: "
                f"{last_error[:200]}. Retrying..."
            )
            await asyncio.sleep(0.5 * attempt)

    raise RuntimeError(f"ffmpeg conversion failed after {max_retries} attempts: {last_error}")
